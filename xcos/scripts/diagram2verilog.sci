...
//target = list index of target superblock within diagram
  ...
  [ko, inports, inports_indices] = find_blocks_of_type('inport', diagram, %inf);
  if ~ko,
    ratel_log(msprintf('error finding inports in %s', diagname)+'\n', [fname, 'error']);
    return;
  end
  ...
  if ~ko,
    ratel_log(msprintf('error generating inport ports for %s', diagname)+'\n', [fname, 'error']);
    return;
  end

  mfprintf(fd, '\n// outports\n'); 
  //find all outports in our diagram (including in all superblocks)
  [ko, outports, outport_indices] = find_blocks_of_type('outport', diagram, %inf);
  if ~ko,
    ratel_log(msprintf('error while finding outports in %s', diagname)+'\n', [fname, 'error']);
    return;
  end
  ...
  if ~ko,
    ratel_log(msprintf('error generating outport ports for %s', diagname)+'\n', [fname, 'error']);
    return;
  end

  mfprintf(fd, '\n// standard ports\n'); 
  //find all inouts in top level of our diagram
  [ko, inouts, inout_indices] = find_blocks_of_type('inout', diagram, 0);
  if ~ko,
    ratel_log(msprintf('error while finding inouts in %s', diagname)+'\n', [fname, 'error']);
    return;
  end
  ...
  if ~ko,
    ratel_log(msprintf('error generating inout ports for %s', diagname)+'\n', [fname, 'error']);
    return;
  end
